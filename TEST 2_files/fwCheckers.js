...
function fw_addNotEmpty(id, message, as_regex)
{
	var elem = $("#" + id);
	if (elem.length)
	{
		...
		if (as_regex && typeof as_regex == "string" && elem.is("input[type=text]:visible"))
		{
			elem.attr("fwRegex", as_regex);
		}
...
function fw_removeRegex(id)
{
	var elem = $("#" + id);
	if (elem.length && $(elem).attr("fwRegex"))
	{
		$(elem).removeAttr("fwRegex");
...
function fw_updateMessage(id, message)
{
...
function fw_checkForm2(vThis, newAlert)
{
	...
	vThis.find("select.fwNotEmpty:visible:not(:disabled), select.fwNotEmpty.fwCheckIfDisabled:visible:disabled, select:not(:visible).fwNotEmpty.fwInputSelectMultiple.fwEnableCheckMultiple").each(function()
	{
		var lo_btnSibling = $(this).siblings("button.ui-multiselect[id=" + $(this).prop("id") + "_ms]");
		if (lo_btnSibling.length && !$(lo_btnSibling).is(":visible"))
		{ 
			return;
		}
	...
	if (!ret)
	{
		vThis = vThis.is("form") ? vThis : vThis.closest("form");
...
function fw_check_select(vThis, isWarningVisu)
{
	...
	$(vThis).siblings("button.ui-multiselect[id=" + $(vThis).prop("id") + "_ms]").each(function()
	{
		$(this).removeClass("fwNotValid");
		if (!ret && isWarningVisu)
		{
