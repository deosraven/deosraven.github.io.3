...
function fwWindow_createIfNeeded(id, width, height, url, titre, closeable, maximizable, resizeable, minWidth, minHeight, maxWidth, maxHeight)
{
	...
	if (!ret.length)
	{
		...
		if (minWidth)
		{
			if (minWidth > widthNav)
			{
				minWidth = widthNav - 20;
			}
		...
		if (minHeight)
		{
			if (minHeight > heightNav)
			{
				minHeight = heightNav - 20;
			}
		...
		if (maxWidth)
		{
			if (maxWidth > widthNav)
			{
				maxWidth = widthNav - 20;
			}
		...
		if (maxHeight)
		{
			if (maxHeight > heightNav)
			{
				maxHeight = heightNav - 20;
			}
		...
		divContentTop.on("mousedown", fwWindow_startDragging);

		$("html").off("mouseup.window", fwWindow_stoptDragging);
		$("html").on("mouseup.window", fwWindow_stoptDragging);
		$("html").off("mousemove.window", fwWindow_drag);
		$("html").on("mousemove.window", fwWindow_drag);
		if (closeable || maximizable)
		{
			if (closeable)
			{
				...
				divClose.on("mouseup.button", function(e)
				{
					e.stopPropagation();
				});
				divClose.on("mousedown.button", function(e)
				{
					e.stopPropagation();
				});
			...
			if (maximizable)
			{
				...
				divMaximize.on("mouseup.button", function(e)
				{
					e.stopPropagation();
				});
				divMaximize.on("mousedown.button", function(e)
				{
					e.stopPropagation();
				});
				divMaximize.on("click", function(e)
				{
		...
		if (url)
		{
			...
			else if (typeof url == "object" && $(url).is("div"))
			{
				divContent.append(url);
				divContent.addClass("fwWindowContentOverflow")
			}
		}
		if (resizeable)
		{
			...
			$("html").off("mouseup.window_resize", fwWindow_stoptResizing);
			$("html").on("mouseup.window_resize", fwWindow_stoptResizing);
			$("html").off("mousemove.window_resize", fwWindow_resize);
			$("html").on("mousemove.window_resize", fwWindow_resize);
		}
		$("#" + FMK_ID_BODY).append(ret);
	}
...
function fwPortletInitProgress(as_source)
{
	var lo_portlet = $("div#portletArea" + as_source);
	if ($(lo_portlet).data("noimage") == "0")
	{
		affichDiv('<div class=\"fwFullWidth fwCenter fw_loading\"></div>', 'portletArea' + as_source);
	}
}

function fwPortletInit(as_source, ai_waitArea)
{
	var lo_portlet = $("div#portletArea" + as_source);
	var ls_init = $("div#portletArea" + as_source).data("init");
	if (typeof ls_init == typeof undefined || ls_init == '' || ls_init != 'true')
	{
		var ls_div = "";
		var ls_waitarea = "";
		if ($(lo_portlet).data("noimage") == "0")
		{
			ls_div = '<div class=\"fwMarginAuto fwCenter fw_loading\"></div>';
		}
		if (ai_waitArea > 0)
		{
			ls_waitarea = "<div class=\"fwFullWidth fwCenter fwVerticalAlignMiddle\" style=\"height: " + ai_waitArea + "px\">";
			ls_waitarea += (ls_div !== '' ? ls_div : '&nbsp;')
			ls_waitarea += "</div>";
		}
		else
		{
		...
		affichDiv(ls_waitarea, 'portletArea' + as_source);
		fwPortletRefreshUrl(as_source, $("div#portletArea" + as_source).data("baseurl"), false);
		$(lo_portlet).data("init", "true");
	}
}

function fwPortletResetProgress(as_source)
{
}

function fwPortletRefresh(as_source)
{
	fwPortletRefreshUrl(as_source, $("div#portletArea" + as_source).data("baseurl"));
}

function fwPortletRefreshUrl(as_source, as_url, ab_refresh)
{
	var lo_portlet = $("div#portletArea" + as_source);
	if (typeof ab_refresh == typeof undefined || ab_refresh == true)
	{
		fwPortletInitProgress(as_source);
	}
	...
	var ajaxRequest = $.ajax(
	{
		...
		{
			$(lo_portlet).html(dataSuccess);
			$(lo_portlet).data("init", "true");
			fwPortletResetProgress(as_source);
		},
		error : function(jqXHR, textStatus, errorThrown)
		{
			$(lo_portlet).html(jqXHR.responseText);
		}
	}).always(function(data)
	{
	});

	addRequestAjax(ajaxRequest);
}

function fwPortletRefreshParam(as_source, as_param)
{
	fwPortletInitProgress(as_source);

	as_param = as_param.trim();
	if ($(lo_portlet).data("baseurl").match(/\?./) || $(lo_portlet).data("baseurl").endsWith('?'))
	{
		as_param = (as_param.startsWith('?') ? ("?" + as_param) : as_param);
	}
	else if (!as_param.startsWith('&'))
	{
		as_param = "&" + as_param;
	}
	...
	var ajaxRequest = $.ajax(
	{
		url : $(lo_portlet).data("baseurl") + as_param + '',
		async : true,
		success : function(dataSuccess)
		{
			$(lo_portlet).html(dataSuccess);
